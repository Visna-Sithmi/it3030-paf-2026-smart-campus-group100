...
const ResourceCataloguePage: React.FC = () => {
  ...
  // Booked slots state (from developer branch)
  const [bookedSlotMap, setBookedSlotMap] = useState<Record<number, BookingSlotDTO[]>>({});
  
  // Availability Popup State (from feature branch)
  ...
  // Holiday Calendar State (from feature branch)
  ...
  // Load booked slots for availability view (from developer branch)
  useEffect(() => {
  ...
  // Get booking state for a resource (from developer branch)
  ...
  // Calendar helper functions (from feature branch)
  ...
  // Availability Popup Content Component (from feature branch)
  const AvailabilityPopupContent = () => {
    ...
    const bookedSlots = bookedSlotMap[selectedAvailabilityResource.id] || [];
    ...
    return (
      <div className="p-5">
        ...
        {/* Time Slots - Compact with booking states */}
